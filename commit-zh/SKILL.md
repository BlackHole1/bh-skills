...
When a branch will be created (`protected=yes` **and** `unborn=no` — an unborn protected branch gets no new branch; the first commit lands on it), add one line under the block naming it, e.g. `将从 main 创建并切换到分支 feat/jwt-token-refresh`. Then branch on the mode:
...
- when step 1 showed `auto_staged=yes` → 已自动暂存全部改动（`git add -A`，覆盖全工作区的新增/修改/删除）
